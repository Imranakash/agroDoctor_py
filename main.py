...
app.config['SQLALCHEMY_DATABASE_URI']='mysql://root:@localhost:3306/farmers'
db=SQLAlchemy(app)
...
class Register(db.Model):
    ...
    nidnumber=db.Column(db.String(50))
...
def edit(rid):
    ...
    if request.method=="POST":
        ...
        nidnumber=request.form.get('nidnumber')
...
def register():
    ...
    if request.method=="POST":
        ...
        address=request.form.get('address')
        farmingtype=request.form.get('farmingtype')     
        query=Register(farmername=farmername,nidnumber=nidnumber,age=age,gender=gender,phonenumber=phonenumber,address=address,farming=farmingtype)
